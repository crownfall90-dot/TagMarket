...
# узкий неразрывный пробел (U+202F) в сумме портала разбирается так же, как обычный
assert _p.parsed_amount({"amount": "3 073.00", "currency": "USD"}) == (3073.0, "USD")
# нулевая/неразборчивая сумма не должна раздувать счётчик пополнений клиента
assert _p.client_deposits_add(db, "CU0", {"amount": "0"}) is None, "нулевой депозит не должен копиться"
assert _p.client_deposits_add(db, "CU0", {"amount": "bad"}) is None, "нечисловая сумма не должна ронять"
