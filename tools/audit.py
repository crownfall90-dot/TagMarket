...
for acc in accounts.load():
    ...
    # 6. Invested сверяем с капиталом, сложенным агентом из истории. Ноль там
    # означает, что исходное пополнение старше доступной истории терминала —
    # тогда сверять не с чем, и Invested остаётся единственной опорой
    st = store.get_state(db, acc["login"]) or {}
    from_hist = float(st.get("capital_hist") or 0.0)
    # Проверка справочная: у части счетов исходное пополнение старше истории,
    # которую хранит терминал, и там капитал по движениям выходит около нуля.
    # Ронять аудит из-за нехватки данных нельзя — просто показываем расхождение.
    if from_hist > cap * 0.5 and acc.get("base") is not None:
        near = money_close(from_hist, cap, max(1.0, cap * 0.02))
        check(True, f"{name}: Invested против истории",
              f"{acc['base']:.2f} и {from_hist:.2f}"
              + ("" if near else "  ← стоит сверить с порталом"))
    else:
        check(True, f"{name}: капитал по Invested",
              f"{cap:.2f} — история пополнений неполная")

    # 7. у свёрнутых месяцев сохранена доходность — иначе история потеряна
