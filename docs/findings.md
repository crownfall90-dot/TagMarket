...
GitHub, в `main` НЕ слита) слита в `main` и закрыла
№7, №11, №13, №14, №17, №19, №21, №22, №24, №26, №28 — отмечены `[x]` ниже,
каждая проверена по коду после слияния, а не по названию коммита. №25 в
предварительный список входил ошибочно: `share()` по-прежнему делает
`int(login)` без проверки типа — пункт остаётся живым.
Живыми на `main` остаются семь: №12, №15, №16, №18, №20, №25, №27.
...
- [x] Исправлено в ветке: REINVEST_PAIR_SECONDS=5 вместо точного равенства времён.
...
- [x] Исправлено в ветке: строгая проверка типа login, bool и float отвергаются.
...
- [x] Исправлено в ветке: money() в app.js обёрнут в try/catch, экран не падает.
...
- [x] Исправлено в ветке: refresh_seconds/server_time теперь используются в app.js.
...
- [x] Исправлено в ветке: `_registration_url` отвергает кавычки, угловые скобки, пробелы, обратные кавычки и не-ASCII в query.
...
- [x] Исправлено в ветке: аннотация приведена к tuple[float, int, int, int].
...
- [x] Исправлено в ветке: точечные ключи удаляются через kv_del_exact.
...
- [x] Исправлено в ветке: token_matches сравнивает байты через compare_digest.
...
- [x] Исправлено в ветке: retry_after считает целое окно перехвата (wait_until).
...
- [x] Исправлено в ветке: day_totals для kind=moves больше не считается впустую.
...
- [x] Исправлено в ветке: список tools/ в README актуализирован.
