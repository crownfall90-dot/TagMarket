...
const state = {data:null, people:null, admin:null, report:null, reportError:'', priceChart:null, notifications:{items:[],unread:0}, view:'overview', login:null, period:'today', kind:'trades', offset:0, from:'', to:'', filters:{overview:{period:'today',from:'',to:''},deals:{period:'today',from:'',to:''},account:{period:'today',from:'',to:''}}, busy:false, currency:null};
...
function accountsView(){const items=state.data.accounts,personal=items.filter(a=>!a.demo&&!a.shared),shared=items.filter(a=>a.shared&&!a.demo),demo=items.find(a=>a.demo),groups=[...new Set(personal.map(a=>a.cabinet||'Без кабинета'))];
 ...
 return header('Счета','',button('add','Добавить счёт','primary','plus'))+(personal.length?balanceCard(personal):'')+groups.map(group).join('')
  ...
  +(demo?`<section class="account-group demo-account-group"><div class="account-group-head"><div class="group-title"><span class="eyebrow">ОБЩИЙ СЧЁТ · ПРОСМОТР</span></div>${button('account-settings','Настроить','secondary small','settings',`data-login="${demo.login}"`)}</div><div class="account-list">${accountCard(demo,1)}</div></section>`:'')
...
function dealHighlights(rep){const items=[['week','Эта неделя'],['lastweek','Прошлая неделя'],['month','Этот месяц'],['all','Всё время']];return `<div class="deal-highlights">${items.map(([key,label])=>{const item=rep.insights?.[key];return `<button type="button" data-action="period" data-value="${key}" class="deal-highlight ${state.period===key?'active':''}"><span>${label}</span><b class="${signedClass(item?.net)}">${item?.available===false?'—':money(item?.net,rep.currency,true)}</b>${item?.pct_capital!=null?`<small>${percent(item.pct_capital)}</small>`:''}</button>`;}).join('')}</div>`;}
function accountPicker(){const acc=currentAccount(),list=state.data.accounts;if(list.length<2)return '';return `<div class="account-chips" role="tablist" aria-label="Счёт">${list.map(a=>`<button type="button" role="tab" data-action="pick-account" data-login="${a.login}" aria-selected="${Number(a.login)===Number(acc?.login)}" class="chip ${Number(a.login)===Number(acc?.login)?'active':''}"><b>${esc(a.strategy||a.name)}</b><small>${esc(a.demo?'общий счёт':String(a.holder||a.cabinet||a.login).split(' ')[0])}</small></button>`).join('')}</div>`;}
function reportFilters(){return `<section class="report-controls panel">${accountPicker()}<div class="report-control report-period">${periodTabs()}</div></section>${customDates()}`;}
function dealsView(){
 ...
 const start=header('Сделки')+reportFilters()+switches;
 if(state.reportError)return start+empty('Не удалось открыть отчёт',state.reportError);
 if(!rep)return start+(state.period==='custom'&&!state.from?empty('Выберите даты',''):skeleton('list',4));
 if(rep.pending)return start+empty('Ожидаем данные','История этого счёта ещё не поступила.');
 if(state.kind==='archive')return start+archive(rep);
 if(state.kind==='moves')return start+movesView(rep);
 const summary=`<section class="panel deals-overview fade-swap"><div><span class="eyebrow">${esc(rep.title)} · ПОСЛЕ КОМИССИИ</span><div class="deals-overview-result">${resultPair(rep.summary.net_income,rep.currency,rep.summary.pct_capital)}</div><p>${countLabel(rep.summary.count,['закрытая сделка','закрытые сделки','закрытых сделок'])}</p></div>${icon('chart')}</section>${dealHighlights(rep)}`;
 return start+summary+(rep.archived?notice('Ранние сделки сохранены итогами по месяцам.'):'' )+tradesTable(rep);
}
...
function accountView(){
 ...
 const intro=`<div class="detail-top"><a href="#accounts" class="text-link back">${icon('back')} Все счета</a>${button('account-settings','Настроить','secondary small','settings',`data-login="${a.login}"`)}</div>`
 ...
 const report=state.reportError?empty('Не удалось открыть историю',state.reportError):rep?.pending?empty('Ожидаем первые данные','Как только терминал прочитает счёт, данные появятся здесь.'):rep?`<section class="panel chart-panel detail-chart fade-swap"><div class="panel-head"><div><span class="eyebrow">ДИНАМИКА</span><h2>${esc(rep.title)}</h2></div><span class="badge">${countLabel(rep.summary.count,['сделка','сделки','сделок'])}</span></div><div class="chart-summary">${resultPair(rep.summary.net_income,rep.currency,rep.summary.pct_capital)}</div>${chart(rep.chart,rep.currency,rep.archived)}${dayExtremes(rep.chart,t?.now,rep.currency)}</section><div class="section-head"><h2>По месяцам</h2></div>${archive(rep)}`:`<section class="panel">${skeleton('chart')}</section>`;
...
function guestCard(g,p){const have=new Set(g.accounts.filter(a=>a.shared).map(a=>String(a.login))),canShare=(p.shareable||[]).filter(a=>!have.has(String(a.login)));return `<section class="panel people-card"><div class="person-head"><div class="avatar">${esc(g.name.slice(0,1).toUpperCase())}</div><div><h3>${esc(g.name)}</h3>${g.since?`<p>${esc(g.since)}</p>`:''}</div></div><p>${countLabel(g.accounts.length,['счёт','счёта','счетов'])}</p>${g.accounts.map(a=>`<div class="settings-row"><p>${esc(a.name)} <small>${a.shared?'Ваш счёт у гостя':'Личный счёт · только просмотр'}</small></p>${a.shared?button('take','Забрать','secondary small','',`data-guest="${g.id}" data-login="${a.login}"`):''}</div>`).join('')}<div class="actions">${canShare.length?button('share-accounts','Открыть счета','secondary small','plus',`data-guest="${g.id}"`):''}${button('guest-detail','Подробнее','secondary small','',`data-guest="${g.id}"`)}${button('revoke-guest','Убрать доступ','danger small','',`data-guest="${g.id}"`)}</div></section>`;}
...
async function navigate(){
 ...
 const next=['overview','accounts','deals','account','people','settings','faq'].includes(view)?view:'overview';
...
document.addEventListener('click',async event=>{const el=event.target.closest('[data-action]');if(!el)return;event.preventDefault();const action=el.dataset.action;if(el.disabled)return;el.disabled=true;try{
 ...
 else if(action==='pick-account'){state.login=Number(el.dataset.login);state.offset=0;state.report=null;state.reportError='';render();await refresh();}
