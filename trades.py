...
def _load_config() -> None:
    """(Пере)читает денежные настройки из окружения в модульные константы.

    Вызывается при импорте и повторно из agent.sync_env() после того, как
    load_dotenv(override=True) обновит os.environ с сервера — иначе смена
    BROKER_FEE/INVESTOR_SHARE/TZ_HOURS/REPORT_FROM на сервере расходилась бы
    молча: os.environ обновлялся, а уже связанные при импорте имена модуля —
    нет, вплоть до ручного перезапуска процесса.
    """
    global SHARE, BROKER_FEE, REPORT_FROM, TZ_HOURS
    # Счёт Amplify: торгуется баланс с плечом, но результат сделок целиком
    # твой — из него лишь вычитается плата платформы (PF Deduction) и
    # корректировки. Сверено с PnL в портале: 13.40 сделок − 3.91 платы =
    # 9.23. Доли прибыли нет.
    ...
    # С какой даты показывать историю. Сделки до неё в базе остаются, но в
    # отчёты не попадают — так отсечку можно двигать, ничего не теряя
    REPORT_FROM = datetime.fromisoformat(os.getenv("REPORT_FROM", "2026-07-01"))
    # Пояс сервера брокера: его время стоит в метках сделок, по нему же
    # считаются дни и границы периодов. Совпадает с московским
...
def _capital_moves(since: datetime) -> float:
    ...
    а не денег — исходный взнос часто старше её, и без него капитал уходил
    в минус.
    """
    total = 0.0
...
def _profit_on_account() -> float:
    ...
    её не выведут или не реинвестируют. Долю брокера считать не нужно: она
    ...
    for r in rows:
        if r["is_closing"]:
            ...
            if is_perf_fee(r):
                left += r["net"]                # доля брокера ушла с баланса
...
def capital() -> float:
    ...
    Считаем из баланса: брокер держит торговый баланс равным деньгам
    инвестора × множитель Amplify, плюс нераспределённый профит (он лежит
    1:1). Отсюда капитал = (баланс − профит) ÷ плечо. Это самая свежая
    величина — она меняется вместе с каждым пополнением и выводом.

    Раньше главным был ручной acc['base'] (Invested, переписанный из портала),
    и капитал на нём «замерзал»: движения до base_at в него не попадали, а при
    повторном вводе Invested это вся история сразу. Счёт, где капитал вырос до
    684.78, продолжал считать от 662.74 — и проценты расходились с отчётом
...
def _stored_capital() -> float:
    """Капитал по всей истории счёта.

    На сервере сделки старше текущего месяца свёрнуты в месячные итоги, и по
    одной таблице deals капитал недосчитывался: у счёта с депозитом в июне
    выходил ноль. Поэтому складываем живые сделки со свёрнутыми transfers —
    при свёртке они сохраняются как раз для этого.
...
def retained() -> float:
    ...
    чистыми: с части сделок брокер свою долю ещё не удержал — PF Deduction
    ...
    left = mine(gross) - BROKER_FEE * max(pending, 0.0)
    ...
    return 0.0 if left < DUST else left


def invested() -> float:
    """Мои деньги на стратегии сейчас = капитал + накопленный профит.
...
def fetch(since: datetime, until: datetime, all_history: bool = False) -> list[dict]:
    """Сделки за период. Границы — по времени портала (UTC).

    all_history снимает отсечку REPORT_FROM: она задаёт, с какого месяца
    показывать отчёты, но к капиталу отношения не имеет — пополнения бывают
    и старше. Считая их с отсечки, мы теряли исходный взнос: у счёта с
    депозитом до июля капитал по истории выходил −0.68 вместо 2500.
    """
    if since < REPORT_FROM and not all_history:
...
def since_ticket(ticket: int) -> list[dict]:
    ...
    Окно раньше было 3 дня: если терминал стоял выключенным дольше (частый
    случай — см. late_note), сделки старше окна, но новее последнего
    отправленного тикета, молча терялись. Депозит на стратегию, сделанный в
    такой момент, никогда не долетал до сервера, и капитал не менялся, хотя
    баланс в MT5 уже вырос. Фильтр по ticket > ticket и так не даёт задвоить
    старые сделки, поэтому нижнюю границу можно не резать вовсе.
    """
    if not HAS_MT5:
