...
def capital() -> float:
    ...
    По умолчанию считаем из баланса: брокер держит торговый баланс равным деньгам
    ...
    Если пользователь вручную привязал Invested из портала, он становится
    опорой до сброса привязки. Дальнейшие пополнения и выводы добавляются к
    нему из истории после даты привязки.

    Нулевой баланс тоже является валидным состоянием; проверяем наличие
